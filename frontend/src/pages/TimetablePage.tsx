...
import { AlertTriangle, CalendarDays, Check, RefreshCw, Sparkles } from 'lucide-react'
import { sectionsApi, timetableApi } from '../services/api'
import type { Section, TimetableGenerateResponse } from '../types'
...
export default function TimetablePage() {
  ...
  const generateMutation = useMutation<TimetableGenerateResponse, any, number[]>({
    mutationFn: (classIds) => timetableApi.generate({ class_ids: classIds, section_ids: [] }),
    onSuccess: (data) => {
      setValidationErrors([])
      setApiError('')
          ...
          {isPending ? (
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-wrap gap-2 border-b border-[var(--color-surface-600)] pb-3">
                {Object.entries(timetables).map(([sectionId, timetable]) => {
